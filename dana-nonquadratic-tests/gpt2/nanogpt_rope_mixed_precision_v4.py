#!/usr/bin/env python
"""
NanoGPT implementation with RoPE (Rotary Position Embedding) and mixed precision support v4.

This implementation stores all parameters in float32 for optimization stability,
but performs most computations in bfloat16 for performance. Sensitive operations
like LayerNorm and softmax remain in float32 for numerical stability.
...
- Parameters stored in float32 for stable optimization
- Matrix multiplications computed in bfloat16 for speed
- LayerNorm, softmax, and loss in float32 for stability
"""
...
class CausalSelfAttention(nn.Module):
    ...
    Parameters stored in float32, computations in bfloat16 except for sensitive operations.
    """
    config: ModelConfig
    init_std: float = 0.02

    def setup(self):
        # Parameters stored in float32 for optimization stability
    ...
    def __call__(self, x, deterministic=True):
        ...
        # Project to q, k, v in float32 (parameters are float32)
        q = self.q_proj(x)
        k = self.k_proj(x)
        v = self.v_proj(x)

        # Reshape to separate heads (still in float32)
...
class MLP(nn.Module):
    ...
    Parameters stored in float32, computations in bfloat16.
    """
    config: ModelConfig
    init_std: float = 0.02
    ...
    @nn.compact
    def __call__(self, x, deterministic=True):
        # Cast input to bfloat16 for computation
        x_bf16 = x.astype(jnp.bfloat16)
        
        # Forward pass in bfloat16
        x_bf16 = self.fc1(x_bf16)
        x_bf16 = nn.gelu(x_bf16, approximate=True)
        x_bf16 = nn.Dropout(rate=self.config.dropout_rate)(x_bf16, deterministic=deterministic)
        x_bf16 = self.fc2(x_bf16)
        x_bf16 = nn.Dropout(rate=self.config.dropout_rate)(x_bf16, deterministic=deterministic)
...
class TransformerBlock(nn.Module):
    ...
    Parameters stored in float32, computations in bfloat16 except LayerNorm.
    """
    config: ModelConfig
    init_std: float = 0.02
    ...
    def __call__(self, x):
        ...
        # Pre-norm architecture - LayerNorm stays in float32, then cast back to bfloat16 for attention and MLP
        x_norm = nn.LayerNorm(dtype=norm_dtype)(x).astype(jnp.bfloat16)
        
        # Attention sublayer (all in bfloat16)
        ...
        # Keep stream in bfloat16 for next block
        return x
...
class GPTWithRoPE(nn.Module):
    ...
    Parameters stored in float32, computations in bfloat16 except sensitive operations.
    """
    config: ModelConfig
    init_std: float = 0.02
    ...
    def __call__(self, x, deterministic=False):
        ...
        # Token embedding (parameters in float32, output in bfloat16)
        x = self.wte(x).astype(jnp.bfloat16)

        # Apply transformer blocks (maintain bfloat16 between blocks)
        ...
        # Final layer norm (in float32)
        x = self.ln_f(x)
        
        # Output projection - keep in float32 for loss computation
        logits = self.head(x)
