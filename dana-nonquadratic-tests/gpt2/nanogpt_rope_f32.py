#!/usr/bin/env python
"""
NanoGPT implementation with RoPE (Rotary Position Embedding) in full float32 precision.

This implementation combines token embeddings with rotary position embeddings,
using float32 for all parameters and computations.

References:
- RoPE: "RoFormer: Enhanced Transformer with Rotary Position Embedding"
  https://arxiv.org/abs/2104.09864

All parameters are stored in float32 for maximum numerical precision.
"""
...
class CausalSelfAttention(nn.Module):
    ...
    All parameters and computations in float32.
    """
    config: ModelConfig
    init_std: float = 0.02

    def setup(self):
        # All parameters stored in float32
    ...
    def __call__(self, x, deterministic=True):
        ...
        if self.config.attention_implementation in ['cudnn', 'xla']:
            ...
            # Fallback to einsum implementation
...
class TransformerBlock(nn.Module):
    ...
    def __call__(self, x):
        ...
        x_norm = nn.LayerNorm(dtype=norm_dtype)(x)
