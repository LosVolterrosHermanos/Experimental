#!/usr/bin/env python
"""
NanoGPT training with Tanea optimizer using pure bfloat16 precision and RoPE.
Based on nanogpt_tanea_tau_stats_bf16.py but with pure bfloat16 and Rotary Position Embedding.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train nanogpt with Tanea optimizer using pure bfloat16 precision and RoPE")
    parser.add_argument(
...
def main():
    """
    Train NanoGPT with Tanea optimizer and collect tau statistics using pure bfloat16 precision and RoPE.
    """
    args = parse_args()
    ...
    results_filename = (
        f"{config['results_dir']}/nanogpt_tanea_results_pure_bf16_rope_{timestamp}_"
