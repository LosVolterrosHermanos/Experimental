#!/usr/bin/env python
"""
NanoGPT training with Tanea optimizer and tau statistics tracking using bfloat16 for matrix multiplies.
Based on nanogpt_tanea_tau_stats.py but with bfloat16 precision for matmuls.
"""
...
import jax
# Enable bfloat16 for matrix multiplications
...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x, deterministic=True):
        ...
        # Cast to bfloat16 for matrix operations, keep float32 for everything else
        ...
        # causal self-attention with bfloat16 matmul
        q_bf16 = q.astype(jnp.bfloat16)
        k_bf16 = k.astype(jnp.bfloat16)
        v_bf16 = v.astype(jnp.bfloat16)
        
        att = jnp.matmul(q_bf16, jnp.transpose(k_bf16, (0, 1, 3, 2))) * (1.0 / jnp.sqrt(k.shape[-1]))
        ...
        # output projection with bfloat16
...
class MLP(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, deterministic=True):
        # Cast to bfloat16 for matrix operations
        x_bf16 = x.astype(jnp.bfloat16)
...
class GPT(nn.Module):
  ...
  def __call__(self, x, deterministic=False):
    ...
    # Final projection with bfloat16
    x_bf16 = x.astype(jnp.bfloat16) 
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train nanogpt with Tanea optimizer and tau statistics using bfloat16 for matrix multiplies")
    parser.add_argument(
...
def main():
    """
    Train NanoGPT with Tanea optimizer and collect tau statistics using bfloat16 for matrix multiplies.
    """
    args = parse_args()
    ...
    logger.info("Using bfloat16 precision for matrix multiplications")
    ...
    for step in pbar:
        ...
        if step in LOG_STEPS:
            ...
            tqdm.write(f"  Precision: bfloat16 matmul\n")
    ...
    results_filename = (
        f"{config['results_dir']}/nanogpt_tanea_results_bf16_{timestamp}_"
