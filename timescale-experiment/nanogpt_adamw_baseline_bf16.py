#!/usr/bin/env python
"""
NanoGPT training with AdamW optimizer using bfloat16 for matrix multiplies.
Based on nanogpt_adamw_baseline.py but with bfloat16 precision for matmuls.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train nanogpt with AdamW optimizer using bfloat16 for matrix multiplies")
    parser.add_argument(
...
def main():
    """
    Train NanoGPT with AdamW optimizer using bfloat16 for matrix multiplies.
    """
    args = parse_args()
    ...
    results_filename = (
        f"{config['results_dir']}/nanogpt_adamw_baseline_bf16_{timestamp}_"
