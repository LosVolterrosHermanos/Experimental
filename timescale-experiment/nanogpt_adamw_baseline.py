#!/usr/bin/env python
"""
NanoGPT training with AdamW optimizer for baseline comparison.
Based on nanogpt_tanea_tau_stats.py but using AdamW optimizer.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train nanogpt with AdamW optimizer for baseline comparison")
    parser.add_argument(
...
def main():
    """
    Train NanoGPT with AdamW optimizer for baseline comparison.
    """
    args = parse_args()
    ...
    results_filename = (
        f"{config['results_dir']}/nanogpt_adamw_baseline_{timestamp}_"
