...
# Simple grid search script for tanea hyperparameters
# Parameters to vary:
# - tanea_g2: {2E-5, 4E-5, 8E-5, 16E-5}
# - tanea_g3: 0 (fixed)
...
total_combinations=$(( ${#TANEA_G2_VALUES[@]} + 1 ))  # +1 for Adam baseline
current=0

echo "Starting simple grid search for tanea hyperparameters"
...
results_dir="simple_grid_search_results_${timestamp}"
mkdir -p "$results_dir"

# Log file for the grid search
log_file="$results_dir/simple_grid_search.log"

echo "Simple grid search started at $(date)" | tee -a "$log_file"
...
for tanea_g2 in "${TANEA_G2_VALUES[@]}"; do
    ...
    echo "Parameters: tanea_g2=$tanea_g2, tanea_g3=$TANEA_G3, momentum_flavor=$MOMENTUM_FLAVOR" | tee -a "$log_file"
    echo "Started at: $(date)" | tee -a "$log_file"
...
echo "=== Simple grid search completed ===" | tee -a "$log_file"
echo "Finished at: $(date)" | tee -a "$log_file"
