#!/usr/bin/env python
"""
NanoGPT training with AdamW optimizer using pure bfloat16 precision.
Based on nanogpt_adamw_baseline.py but with nearly all computations in bfloat16.
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x, deterministic=True):
        ...
        att = jnp.matmul(q, jnp.transpose(k, (0, 1, 3, 2))) * (jnp.bfloat16(1.0 / jnp.sqrt(k.shape[-1])))
...
class GPT(nn.Module):
  ...
  def setup(self):
    ...
    self.wpe = nn.Embed(self.config.block_size, self.config.n_embd, dtype=jnp.bfloat16)
  ...
  def __call__(self, x, deterministic=False):
    ...
    tok_emb = self.wte(x)
    x       = (tok_emb + pos_emb).astype(jnp.bfloat16)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train nanogpt with AdamW optimizer using pure bfloat16 precision")
    parser.add_argument(
...
def main():
    """
    Train NanoGPT with AdamW optimizer using pure bfloat16 precision.
    """
    args = parse_args()
    ...
    logger.info("Using pure bfloat16 precision for all computations")
    ...
    for step in pbar:
        ...
        if step in LOG_STEPS:
            ...
            tqdm.write(f"  Precision: pure bfloat16\n")
    ...
    results_filename = (
        f"{config['results_dir']}/nanogpt_adamw_baseline_pure_bf16_{timestamp}_"
