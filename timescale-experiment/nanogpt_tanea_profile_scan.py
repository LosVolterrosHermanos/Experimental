...
Multi-GPU data parallel version for 4 GPU systems with scan-based training blocks for optimal performance.
Based on nanogpt_tanea_profile.py with jax.lax.scan optimization.
"""
...
logging.basicConfig(
    ...
    filename='train_multi_gpu_scan.log',  # Separate log file
    filemode='w'
)
...
def parse_args():
    ...
    parser.add_argument(
        "--scan_batch_size", type=int, default=100,
        help="Number of steps to process in each scan block for optimal performance"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--profiler_dir", type=str, default="/tmp/nanogpt_profile_scan",
...
def main():
    """
    Train NanoGPT with Tanea optimizer using mixed precision, RoPE, multi-GPU data parallelism, and scan optimization.
    """
    args = parse_args()
    ...
    logger.info(f"Scan batch size: {args.scan_batch_size} steps per block")
    ...
    logger.info(f"Scan optimization: {config['scan_batch_size']} steps per block")
    ...
    total_blocks = (config["train_steps"] + config["scan_batch_size"] - 1) // config["scan_batch_size"]
    ...
    try:
        for block_idx in pbar:
            ...
            block_size = min(config["scan_batch_size"], remaining_steps)
            ...
            # Run the scan block (this is where the magic happens)
            state, block_losses = train_block_fn(state, data_batch)
            
            # Calculate average loss for this block
            avg_block_loss = jnp.mean(block_losses)
            
            # Update counters
            current_step += block_size
            tokens_processed += block_size * step_tokens
            
            # Calculate performance metrics
            elapsed = time.time() - start_time
            overall_tokens_per_sec = tokens_processed / elapsed if elapsed > 0 else 0
            
            # Update progress bar with average loss and tokens/sec
            pbar.set_postfix(
                loss=f"{avg_block_loss:.4f}", 
                step=f"{current_step}/{config['train_steps']}",
                **{"token/s": f"{overall_tokens_per_sec:,.0f}"}
            )
    ...
    logger.info(f"Completed {current_step} training steps in {block_idx + 1} scan blocks")
    return None
